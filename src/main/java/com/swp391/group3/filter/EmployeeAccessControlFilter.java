 ...
 */
package com.swp391.group3.filter;
 ...
 * @author Dell
 */
public class EmployeeAccessControlFilter implements Filter {
    ...
    public EmployeeAccessControlFilter() {
    }

    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("EmployeeAccessControlFilter:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("EmployeeAccessControlFilter:DoAfterProcessing");
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        System.out.println("EmployeeAccessControlFilter: " + uri);
        int lastIndex = uri.lastIndexOf("/");
        String action = uri.substring(lastIndex + 1);
        HttpSession session = httpReq.getSession(false);
        ServletContext context = httpReq.getServletContext();
        Properties adminAccessControl = (Properties) context.getAttribute("ADMIN_ACCESS_CONTROL");
        String accessRole = adminAccessControl.getProperty(action);
        System.out.println("accessRole: " + accessRole);
        if (session != null) {
            UserDTO user = (UserDTO) session.getAttribute("USER");
            if (user != null && user.getRoleId() == AppConstants.UserRoles.CUSTOMER) {
                session.invalidate();
                ...
                } else if (accessRole.contains(String.valueOf(user.getRoleId()))) {
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("EmployeeAccessControlFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("EmployeeAccessControlFilter()");
        }
        StringBuffer sb = new StringBuffer("EmployeeAccessControlFilter(");
