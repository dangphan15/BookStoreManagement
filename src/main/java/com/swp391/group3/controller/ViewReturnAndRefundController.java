 ...
 * @author QuanMX
 */
public class ViewReturnAndRefundController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = AppConstants.ReturnAndReFundFeatures.RETURN_AND_REFUND_ORDER_PAGE;
        try {
            String status = AppConstants.OrderStatus.RETURN_REFUND;
            //1. Call DAO
            OrderDAO orderDao = new OrderDAO();
            List<OrderDTO> orders = orderDao.getAllOrdersByStatus(status);
            request.setAttribute("ORDERS", orders);
        } catch (ClassNotFoundException | SQLException | NamingException ex) {
            Logger.getLogger(ViewCompletedOrderController.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
