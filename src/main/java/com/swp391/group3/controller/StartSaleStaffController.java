 ...
 * @author Dell
 */
@WebServlet(name = "StartSaleStaffController", urlPatterns = {"/admin/StartSaleStaffController"})
public class StartSaleStaffController extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String url = AppConstants.LoginFeatures.SALE_STAFF_PAGE;
        try {
            OrderDAO orderDao = new OrderDAO();
            List<OrderDTO> orders = orderDao.getAllOrders();
            request.setAttribute("ORDERS", orders);
        } catch (SQLException ex) {
            Logger.getLogger(StartSaleStaffController.class.getName()).log(Level.SEVERE, null, ex);
